# w34_audit — hostile audit of wave-33: ALL core claims CONFIRMED; the TIE-CHART sensitivity is the big structural find (codex, 2026-06-12)
# Brief: notes/briefs/w34_audit.md. Long form: experiments/out/w34_audit/audit.md.

VERDICT TABLE: A1 CONFIRMED-with-tie-caveat (0.92) | A2 CONFIRMED (0.88) | A3 UNRESOLVED (0.84;
data through k=40 favors limit 2 but no certificate) | A4 CONFIRMED, constants TIGHT (0.90) |
A5 CONFIRMED (0.96) | A6 orchestrator's convexity-refutation CONFIRMED (0.98; "directionally
wrong: Jensen gives lower control, not upper") | A7 P=LB/BL=I equivalence + anchors CONFIRMED
(0.93; BL=I forces the rank conditions; det(P_U P_U^T) = det(L_U)^2 det(B B^T) validates minor scans).

THE BIG STRUCTURAL FIND (audit "additional point 2"): max-volume TIE bases change everything
quantitative. A1's transverse pair: intended chart ratio m(1+4a^2), but the signed-row tie
charts give ratio EXACTLY 1. A5's dense-pair k=7: identity chart 17/8; both signed-row tie
charts give ratio 1 (and no tie exceeds 17/8). CONSEQUENCE (orchestrator): SF as stated is
ambiguous over tie choice. If the downstream chain (tax -> displacement -> face -> L4) only
needs SOME max-volume basis (orchestrator's choice), the right statement is likely
EXISTS-a-max-volume-basis SF <= C delta — and in every audited "bad" instance the best tie
has ratio 1. The moderate-delta blow-ups may be artifacts of the WORST tie. Wave-35 question:
(i) does w30's displacement reduction tolerate an arbitrary/chosen max-volume basis? (ii) is
there a deterministic tie-break (prefer signed-face rows as representatives) under which SF
holds with small C? w34_halfcex (in flight) already reports best/worst tie at cap=1/2.

Also confirmed: A2's mechanism ("BL=I sees only aggregate +/- mass over duplicate columns");
unbalancing makes the ratio WORSE. A4's certificates 1+4a^2 / 1+3a^2 are tight at the audited
supports. Minor: w34_audit brief's experiment paths lacked the agent-A/explorations prefix
(worker found them anyway); A1 formula needs its sign-chamber hypotheses (m/2 >= 2ac, a <= 1/2).
---
