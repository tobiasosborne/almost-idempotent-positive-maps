# w31_tax — DIED-AT: narrowed to the SIGNED-FACE EXCESS (codex, 2026-06-11)
# Brief: /tmp/codex-sigma-wall/w31_tax/brief.md. Long form: experiments/out/w31_tax/proof.md.

VERDICT: DIED-AT. I did not prove the tax and did not find a counterexample.

Long-form artifact is in [proof.md](/tmp/codex-sigma-wall/w31_tax/proof.md). The narrowed open core is the signed-face excess estimate

\[
\sum_j (P_{u_sj})_+
\left(\sum_{t\ne s}(-a_t(j))_+-(1-a_s(j))\right)_+
\le C_{\rm sf}\delta.
\]

The scalar deficit part is already controlled by `2 delta`; this remaining term is exactly where the transverse-pair and random small-delta audits carry the tax.

Saved artifacts:
- [tax_audit.py](/tmp/codex-sigma-wall/w31_tax/tax_audit.py)
- [tax_audit_summary.txt](/tmp/codex-sigma-wall/w31_tax/tax_audit_summary.txt)
- [lp_rank_stress.py](/tmp/codex-sigma-wall/w31_tax/lp_rank_stress.py)
- [lp_rank_stress_summary.txt](/tmp/codex-sigma-wall/w31_tax/lp_rank_stress_summary.txt)
- [progress.md](/tmp/codex-sigma-wall/w31_tax/progress.md)

Verification passed:
`python3 -m py_compile tax_audit.py lp_rank_stress.py`
