# w21_recode — L1+L2 PROVED (mass-removed recoding; honest two-scale form) [pending w21_recode_audit]
# Brief: /tmp/codex-sigma-wall/w21_recode/brief.md. Long form: experiments/out/w21_recode/proof.md.

VERDICT: **L1+L2 PROVED in the honest two-scale form; L3 NOT ASSEMBLED**.

Long-form proof is in [proof.md](/tmp/codex-sigma-wall/w21_recode/proof.md:1). I did not create `answer.md`.

Artifacts:
- [recode_removed_mass_checks.py](/tmp/codex-sigma-wall/w21_recode/recode_removed_mass_checks.py:1)
- [recode_removed_mass_summary.txt](/tmp/codex-sigma-wall/w21_recode/recode_removed_mass_summary.txt:1)
- [recode_removed_mass_results.json](/tmp/codex-sigma-wall/w21_recode/recode_removed_mass_results.json:1)
- [progress.md](/tmp/codex-sigma-wall/w21_recode/progress.md:1)

Key constants/results:
- Recoding distance: `||P0 - P0'||_{∞,1} <= 2 * (sum_s q_s + q')`, so `A = 2`.
- Survivor mass: `mu(P0') >= min survivor original mass`, with survivor relative coordinate shift exactly `q_s`.
- Transient face drop: coefficient row shift `= 2 r_i`.
- w19 first-order lemma explicitly applies at every recoded boundary H-M base because it quantifies over all H-M points.

Numerics:
- `python3 -m py_compile recode_removed_mass_checks.py` passed.
- Re-ran the repaired stress checks.
- The old w20 failures remain as obsolete diagnostics: `eps/(mu_after/8)=62.7..313`.
- The repaired removed-mass checks pass: `eps/(2Q) < 1`, survivor shifts `<= q_s`, face shifts `<= 2r_i`.
- Mixed-rate degeneration tests: `12` cases, `0` nonexpansive failures, `0` delta-nonincrease failures.
- w21 sharp boundary events: local fixed-base max ratio `0.0`; transition max ratio `2.0000000000392326`, with top sharp cases all in `t/min_positive_entry >> 1` boundary windows.

L3 status: the remaining estimate is still the finite-jet normal projection bound after all H-M rebasings. Calibration recorded in `proof.md`: `P(L1+L2 survive audit)=0.78`, `P(L3 assembles from present ingredients)=0.34`.
---
