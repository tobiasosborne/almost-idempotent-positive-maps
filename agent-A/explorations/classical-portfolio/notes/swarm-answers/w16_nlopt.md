# w16_nlopt — BARRIER CROSSED at large delta / tiny height (codex, 2026-06-11) [pending w16_cert_audit]
# Brief: /tmp/codex-sigma-wall/w16_nlopt/brief.md. Artifacts: experiments/out/w16_nlopt/.

VERDICT: BARRIER CROSSED.

Verified best saved instance: [w16_best_certificate.json](/tmp/codex-sigma-wall/w16_nlopt/w16_best_certificate.json), [w16_best_matrix.json](/tmp/codex-sigma-wall/w16_nlopt/w16_best_matrix.json), [w16_verdict.md](/tmp/codex-sigma-wall/w16_nlopt/w16_verdict.md).

Key certificate numbers:

```text
n = 7, k = 4
delta = 0.22840025035566297
tau = 0.4779123877403294
...
Triple-check from disk:

```text
R Lambda - I residual = 2.22e-16
P^2 - P residual = 2.22e-16
row-sum residual = 2.22e-16
```
...
Best frontier in this run: `(n,k)=(7,4)` crossed with `delta/sigma_tilde^2 = 0.37843968308661874`. Small random samples for `(6,3)`, `(8,4)`, `(10,4)` found no hidden PASS; `(12,4)` found PASS but only `sigma_tilde/tau = 0.00416`.

Calibrated `P(hidden => sigma_tilde <= sqrt(delta) is true) = 0.01` for the `C0=1` barrier, with the residual only reflecting numerical-certification caution.

Single most informative next experiment: rationalize or interval-certify the saved factorization, then continue the active-set system at the row-1 `rho` threshold to maximize `sigma_tilde/tau` while preserving hiddenness.
---
## Appendix: w16_verdict.md (worker long-form, verbatim)
