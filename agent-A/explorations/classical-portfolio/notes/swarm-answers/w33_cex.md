# w33_cex — DIED-AT: no counterexample; all transverse-stacking mechanisms BOUNDED; candidate dual-flow obstruction lemma (codex, 2026-06-12)
# Brief: notes/briefs/w33_cex.md. Long form: experiments/out/w33_cex/proof.md.
# Orchestrator review (Claude, 2026-06-12): section-3 exact family hand-verified (BL=I, row sums, nu = a/(1+4a^2) = delta, SF = ma) — algebra correct.

VERDICT: DIED-AT, with bounded search evidence.
- Exact transverse-pair family: SF/delta -> 1 exactly (m -> 1, a -> 0); duplicate stacking q=2..25 leaves the ratio UNCHANGED (B_0's positive mass is fixed: splitting it among copies cannot raise SF).
- w25 split-block 1/delta coefficient pumping DIES in the recomputed max-volume chart (SF = 0 for all tested eps).
- Graph stacks (paths/cycles/stars/complete, fixed-L geometry-free LPs via BL=I): flat at ~0.99; at minimum negativity cap, SF/cap ~= 1.
- Max-volume TIE charts are a real effect: path-tie families climb 1.25 -> 1.91 (k=4..20), strongly suggesting limit 2, not growth.
- 240 random small-delta conjugated instances: best 0.901.
- Calibration: P(transverse-stack cex) ~ 0.04, P(some other small-delta cex) ~ 0.12, P(diagnosis survives audit) ~ 0.76.
- CANDIDATE TRANSVERSE-FLOW LEMMA (the wall, = the charging argument in dual form): signed-face atoms are edges, BL=I is a dual-flow constraint; a positive row-0 circulation on signed atoms must be balanced by opposite signs in foreign dual rows; duplication splits the dual flow but cannot increase row-0 mass. Conjectured: SF <= (1+O(a))delta intended chart, <= (2+O(a))delta tie charts.
- INFRA: gurobi blocked INSIDE the codex sandbox (HostID license mismatch; saved failures); SciPy HiGHS used — fixed-L searches are LINEAR once BL=I enforces idempotence.
- NOT YET TESTED (orchestrator note): the depth-m hierarchical tower with amplitude-1 covering rows (a_s = a_r = 1, a_q = -1) — all tested atoms had small transverse amplitude or were flat stacks; the tower is the named residual refutation route.
---
