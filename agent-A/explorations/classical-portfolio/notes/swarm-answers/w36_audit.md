# w36_audit — w35 pair CONFIRMED (constants, routing, selection, chain) BUT B6: the exact-tie collapse is FRAGILE — theta < 1 slack is MANDATORY (codex, 2026-06-12)
# Brief: notes/briefs/w36_audit.md. Long form: experiments/out/w36_audit/audit.md.
# Orchestrator consequence analysis appended below the verdict summary.

VERDICTS: B1 constants chain CONFIRMED w/ minor threshold correction (0.92; the delta_sm
"equivalent" is only a conservative sufficient bound). B2 routing-caveat consistency
CONFIRMED (0.94; w35_charge's deficit usage is theta=1-only, valid there). B3 end-to-end
CONFIRMED AS CONDITIONAL (0.86; several upstream links are DIED-AT used conditionally —
honest status preserved). B4 selection + stress checks CONFIRMED (0.99; tie enumeration
exhaustive; independent Gram verifier agrees). B5 (CHARGE) CONFIRMED but MOSTLY TAUTOLOGICAL
(0.90; without structure on q it is essentially the SF bound itself — provers should attack
SF_s(U*) <= C delta directly, not treat (CHARGE) as an easier intermediate). B6 CONFIRMED —
THE HIT (0.99):

THE PERTURBED STAIRCASE (exact rational family, independent check m=5, eps=1/1000):
x_pm = (1-eps) e_0 + sum (eps/(2m) pm sigma_t/2) e_t; delta = 1/2; the identity chart is the
UNIQUE exact max-volume basis (all others at volume 1-eps) and carries ratio m - 3eps.
So at delta_0 = 1/2 the EXISTS-EXACT-max-volume (theta=1) SF is FALSE with unbounded rank.
The good charts survive at volume ratio exactly 1-eps with ratio 1.

ORCHESTRATOR CONSEQUENCES:
(1) theta is LOAD-BEARING AND QUANTITATIVE. For this family the good charts are included iff
    theta <= 1-eps. Since eps < m/(2m+1) < 1/2 is forced, a FIXED theta <= 1/2 always
    includes them; for any theta > 1/2 the adversary picks eps in (1-theta, 1/2) and kills
    the contract at delta_0 = 1/2. So the registry contract should FIX theta = 1/2 (cost:
    A = 2 in the constants chain), not leave theta -> 1.
(2) w35_charge's theta=1 selection U* is the WRONG CLASS for the final theorem — the
    selection must run over the theta-quasi class. Its stress results remain valid as
    exists-statements (the good charts are in the theta=1/2 class).
(3) The delta_0 <= 0.3 target is NOT refuted by this family (its delta is pinned at 1/2),
    but wave-37 provers must work in the theta=1/2 class from the start, and a small-delta
    perturbed analog should be sought before trusting the contract.
---
## Appendix: audit.md (verbatim)
