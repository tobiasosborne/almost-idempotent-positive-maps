# w22_jet — DIED-AT J1: support-addition arcs refute the fixed-stratum normal bound (codex, 2026-06-11)
# Brief: /tmp/codex-sigma-wall/w22_jet/brief.md. Long form: experiments/out/w22_jet/proof.md.

VERDICT: DIED-AT J1.

I found an explicit boundary-stratum degeneration: exact nonnegative H-M support-addition arcs have `delta = 0` to all orders, but their first jet is not tangent to the fixed recoded boundary stratum. So `||(C,D)_{perp M}||^2 <= L * delta` cannot hold as stated.

Long-form writeup is in [proof.md](/tmp/codex-sigma-wall/w22_jet/proof.md:1). Numerics and code are saved in:

- [jet_counterexample.py](/tmp/codex-sigma-wall/w22_jet/jet_counterexample.py:1)
- [numerics_summary.txt](/tmp/codex-sigma-wall/w22_jet/numerics_summary.txt:1)
- [jet_counterexample_results.json](/tmp/codex-sigma-wall/w22_jet/jet_counterexample_results.json:1)
- [progress.md](/tmp/codex-sigma-wall/w22_jet/progress.md:1)

Verification run passed: `python3 -m py_compile jet_counterexample.py` and `python3 jet_counterexample.py`.
