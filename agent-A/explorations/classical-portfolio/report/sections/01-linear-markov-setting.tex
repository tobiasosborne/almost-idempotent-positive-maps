...
The all-ones column is \(\one\), and
\[
...
stability are equivalent up to universal constants: \(Q\) row-stochastic
with \(\lVert Q^2-Q\rVert \leq \eta\) gives
\(P=\theta(2Q-1)\) signed affine retraction with
\(\lVert P-Q\rVert \leq C\eta\) and neg mass
\(\delta\leq C\eta\), and conversely row-normalising \(p_i^+\) gives
\(Q\) with \(\lVert P-Q\rVert \leq 2\delta\),
\(\lVert Q^2-Q\rVert \leq 6\delta+4\delta^2\).
\end{quote}
In this report the matrix norm in that contract is the maximum row-sum
norm \(\lVert\cdot\rVert_{\infty\to\infty}\), and all constants are
universal.  Thus the original dimension-free problem for row-stochastic
\(Q\) with small \(\eta(Q)=\lVert Q^2-Q\rVert_{\infty\to\infty}\) may be
studied through exact signed affine retractions \(P\) with small
\(\delta=\max_i\nu_i\).
\end{proposition}
...
retraction \(P\) with neg mass \(\leq\delta\leq\delta_0\) having
representatives \(r^1..r^m\) pairwise \(2\rho\)-separated, each
\((\rho,\kappa)\)-exposed (disjoint clusters \(U_a\)), with every
off-cluster row within \(\gamma\) of \(\conv\{r^a\}\), admits a
