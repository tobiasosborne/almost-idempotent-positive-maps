...
q_a >= -C eta ||a||^2 1 (from lem-square-hole-almost-positive) plus approximate invariance bounds
omega(q^+), and faithfulness upgrades small expectation to small operator norm at the cost of 1/lambda
(the conditioning of omega). Polarisation gives the bilinear hole bound for the [[def-positive-unital-map]]
Phi. Report `thm:faithful-approx`; source report/sections/06b-faithful-invariant.tex.
