...
theorem says that every finite-dimensional approximate $C^{*}$-algebra is
$O(\eps)$-isomorphic to a genuine $C^{*}$-algebra, with a dimension-free implicit
constant~\cite[Theorem \texttt{th\_main}]{Kitaev2024}. The Jordan analogue we are
...
recorded as Theorem~1.1 of \cite{ChuRusso2016}: derivations from $J$ to $M$ are
inner. The second part is the Jordan analogue of the second Whitehead lemma,
Theorem~1.2 of \cite{ChuRusso2016}, attributed there to \cite{Penico1951}: Jordan
$2$-cocycles are coboundaries. The vanishing of $H^{1}$ and $H^{2}$ is what
would make the exact correction step possible \emph{in principle}; the issue is
...
claims have not yet been independently byte-matched against extracted primary
