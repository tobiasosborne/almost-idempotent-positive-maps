...
Suppose there are a finite-dimensional $C^{*}$-algebra $D$, a unital
order-isometric Jordan embedding $j\colon\Bsa\to D_{\mathrm{sa}}$, and a unital
positive map $C\colon D_{\mathrm{sa}}\to\Bsa$ with $\Phimap=Cj$, such that the
lift $F=jC$ extends to a unital completely positive map $D\to D$ with
