...
\status{(proved, conditional on the lifted-UCP hypothesis)}\afbadge{thm-dilation-compatible}
...
that structure, and the $\eps$-JB estimates restrict along the embedding. Each of
the three steps is independently machine-validated: idempotence inheritance
$\norm{\Phimap^2-\Phimap}\le\norm{F^2-F}$
(\aflink{lem-idempotence-inheritance}{af}), the spectral intertwining
$\theta(2F-\Id)\,j=j\,\theta(2\Phimap-\Id)$
(\aflink{lem-intertwine-spectral-idempotent}{af}), and the $C^{*}\!\to$JB
symmetrisation that supplies the $O(\eta)$ crux
(\aflink{lem-cstar-sa-to-epsjb}{af}).
\end{proof}
